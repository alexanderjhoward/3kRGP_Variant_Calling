...
# Download BAM file and create missing index
wget http://s3.amazonaws.com/3kricegenome/Nipponbare/${BAM}.realigned.bam
samtools index ${BAM}.realigned.bam

# Subset out regions of interest from alignment file (aligned relative to Nipponbare)
samtools view -b -h ${BAM}.realigned.bam -M -L ../1_Reference_Genome/Output/genes.bed > Output/BAM/${BAM}_subset.bam
